...
function history_none() {
    ...
    //window.location.hash = "..."はURLの#部分の後を取得する。若しくは設定するプロパティ。今回は設定。
    window.location.hash = "no-back-button";
    window.onhashchange = function () {
        //window.onhashChangeはhash即ちURLに変化が発生したときにイベントが動作する。
        window.location.hash = "no-back";
        //どのURLに行っても#no-backがつく。
    }
    history.pushState(null, null, null);
    //history.pushState(state, title, url)
    //上の形式で履歴に指定したURLを追加する。今回は何も追加しないようになっている。
}
function menu_animate(navi) {
    var ul_navi = "ul." + navi + " li";
    //未使用。必要だと思って書いたのだろうか？
    $("ul.navi li:not(.disable)").hover(function () {
        //hoverは(mouseenterの時の動作,mouseleaveの時の動作)
        //<ul class="navi"><li></li></ul>で<li></li>の内でdisableでないものの上にポインタが乗ったときに、
        $(">ul:not(:animated)", this).slideDown("fast");
        //>ulとすることでul内部のulについて言及している。
        //アニメーション中ではない要素(this)をslideDown(素早く)する。
    },
        function () {
            $(">ul", this).slideUp("fast");
            //mouseが離れたときはslideupさせる。
        });
}
function jyuchu_itiran(item1, item2) {
    $("#" + item1 + " ,#" + item2).on("click", "tr", function () {
        //$(#item1,#item2).on("click","tr")とすることでテーブル行に対してonclickイベントを設定する。ちなみに.clickとするのはJQueryの記法なのでonで統一するのが楽かもしれない。
        //なお、これとソースコードをみてもitem1,item2が何を意味するかは不明。
        //https://qiita.com/zuchi190/items/d1e2a612e9ad9e4400d8
        /* $(this).siblings().removeClass('active'); */
        $("#" + item1 + " ,#" + item2).find('.active').removeClass('active');
        //active状態なモノを探しclass="active"となっている部分をclass=""に変更する。
        $(this).addClass('active');
        //$thisにactiveであるというクラス属性を付け足す。
    });
}
function comment_show() {
    $('.comment button').on('click', function () {
        //class="comment button"をクリックしたときに
        $(this).blur();
        //フォーカスを外す
        $('.comment_over').fadeIn(200);
        //class="comment_over"をフェードインする(200の速さで)
    });
    $('.comment_over').on('click mouseleave', function () {
        //clickした状態からマウスが離れたときに
        $(this).fadeOut(100);
        //coment_overのモノをフェードアウト(100の速さで)
    });
}
function comment_show_update() {
    $('.comment button').on('click', function () {
        $(this).blur();
        var offset = $(this).offset();  //これで$thisの「画面全体から見た」位置座標が取得できる。
        var coH = $('.comment_over').height();  //これでclass="comment_over"の「要素の」高さが取得できる。
        $('.comment_over').css({ top: (offset.top - coH), left: offset.left - 30 }).fadeIn(200);
        //comment_overのモノのcssを一番上から要素の高さ分引いた高さに、左から-30の位置にフェードイン
    });
    $('.comment_over').on('click mouseleave', function () {
        $(this).fadeOut(100);
    });
...
function setbox_position(table, num) {
    var td = $('#' + table + ' th:nth-child(' + num + ')'); //id={?}のテーブル（を想定している）のthのnum番目を変数tdとする。
    var setbox = $('.set_box'); //class="set_box"のもの全て？ 一意に定まる？
    var td_offset = td.offset();    //変数tdの位置座標
    var td_w = td.outerWidth(); //変数tdの外部横幅(border、paddingを含む)を取得します。
    var setbox_w = setbox.outerWidth(); //変数set_boxの外部横幅(border、paddingを含む)を取得します。
    setbox.css({ left: td_offset.left + (td_w / 2) - setbox_w / 2 });
    //class="set_box"のモノの左からの配置位置をtd.offset()の左からの座標+外部横幅/2+setboxの外部横幅/2に設定する。
...
function initial_hide(item) {
    ...
    //class="{$item}"なモノを隠す。
    width_T("jyuchu");
    //width_Tは下の方で定義される関数でそれの値として"jyuchu"を入力した。
}
function bukken_table(item) {
    $('#' + item).on("click", "tr", function () {
        rINX = $('#' + item + " tr").index(this);  //要素の番号を求める
        $edit_obj = $(this);    //jQueryオブジェクトのメソッドを使いたいときは$(this)
        $cur_tr = this; //DOMエレメントのプロパティやメソッドを使うときはthis
        if (item == "jyuchu") {
            $jyuchu_table = 0;   //jyuchu  物件
        } else $jyuchu_table = 1;
        $(this).siblings().removeClass('active');   //siblings()は各要素の兄弟要素を全て返す。そしてactiveクラスを削除する。
        $(this).addClass('active'); //activeクラスを付け加える。
        if ($(this).children().eq(9).text() != "東京") {
            //thisの子要素の内0から数えて9番目の要素のtextが東京ではない場合、
            $('#ikanButton').addClass('ikan_disabled');
            //id="ikanButton"に対してclass="ikan_disable"を付け足す。
        }
        else {
    ...
    $('#' + item).on("click", "tr", function () {
        ...
        if (rhinmokuINX != 0) {
            ...
            if (item == "hinmoku_itiran_3") {
                rsyoINX = rhinmokuINX;  //rsyoINXは小のクリック行
            }
            //立替金用の表示
            ...
            //rows[0]ならば表の1行目に対する参照。今回はrhinmokuINX行目
            //cells[1]は表の2行目のセルへの参照。余談ですが、cells.lengthとすると列の数を返す。
            //テーブル要素に付けたidにのみこの関数を適用するならば、$tblがtableになることを保証できる？
...
function set_hinmoku_jyuchu() {
    var idx = 0;
    if ($jyuchu_table == 0) {
        //定義より$jyuchut_tableが立替のとき
        idx = (rINX - 1) * NAME_SUU;    //(行全体の数値-1)*(name配列一行の定数＝23)
    } else {
        idx = (rINX - 1) * TATEKAENAME_SUU; //(行全体の数値-1)*22
    }
    var t = document.getElementById("hinmoku_itiran_1");    //品目一覧1の要素（多分テーブル)を取得する。
    tmp = t.rows[rdaiINX].cells[0]; //大のクリック行の一番目のセルを取得する。
    document.form1.elements['jyuchuname[]'][idx].value = tmp.innerHTML;
    //form1の要素[x][y]の値に大のクリック行の一番目のセルの内容を代入する。
    ...
    tmp = t.rows[rsyoINX].cells[1]; //小のクリック行の2番目のセルを取得する。

    document.form1.elements['jyuchuname[]'][idx + 3].value = tmp.innerHTML;
    $('#select_list').dialog("close");
    //ダイアログを閉じる。#select_listというものはダイアログらしいが、どのような形状なのだろうか。
}
/*
...
function get_jyuchu_zei() {
    ...
    zei_ritu = $('select[name="jyuchu_zei"]').val();    //name="jyuchu_zei"に入力された値を取得する。
    zeiritu = 1 + (zei_ritu / 100); //例えば税率10%で税抜価格が100円ならば、小計は110円、内10円は消費税。

    return zeiritu;
}
/*
*  外注の消費税の取得
*  受注と外注で消費税が変わるってどんな状況？
*/
...
function set_jyuchu_kingaku() {
    ...
    suu = parseInt(document.form1.elements['jyuchuname[]'][idx + 5].value); //document.form1.elements['jyuchuname[]'][idx + 5].valueをINT型に変換したものを返す。
    //312行目でsuuが上書きされているがそれまでにこのsuuを使っている？ 必要だった？
    suuval = document.form1.elements['jyuchuname[]'][idx + 5].value;    //これを先に定義してsuu=parseInt(suuval); とした方がいいのではないか？
    //カンマを消す
    suu = parseInt(suuval.replace(/,/g, ""));   // /,/gは正規表現であり、これが無いと「内部のカンマを全部消す」という結果にならない。一番最初のカンマを消すだけにとどまる。
    tankaval = document.form1.elements['jyuchuname[]'][idx + 7].value;
    tanka = parseInt(document.form1.elements['jyuchuname[]'][idx + 7].value);   //316行目で上書きされているがこれは必要だったか？
    //カンマを消す
    tanka = parseInt(tankaval.replace(/,/g, ""));
    kekka = suu * tanka;    //suu*tankaが成立するということはsuuもtankaも配列ではないはず。つまり312,316行目でのは追加ではなく、上書きで間違いないと思われる。
    //カンマ付ける
    kekkatmp = kekka.toLocaleString();  //下記のように後から.toLocaleString()を付けるのではダメだったのだろうか。
    if (!isNaN(suu)) {
        //suuがNaNではないならば。。。
        document.form1.elements['jyuchuname[]'][idx + 5].value = suu.toLocaleString();
        //suuにtoLocaleStringで三桁区切りカンマを付けたものをフォームに代入。
    }
    if (!isNaN(tanka)) {
    ...
    if (!isNaN(kekka)) {
        ...
        document.form1.elements['jyuchuname[]'][idx + 8].value = "";    //kekkaが数値以外の場合の処理。
    }
}
/*
*受注の小計を計算
...
function set_jyuchu_syokei() {
    rw = getRowNum("jyuchu");
    //行数を取得する？ getRowNumが調べてもjavascriptでもJQueryでも単体では出てこない。
    ...
    var idx = 0;
    var tmp = 0;
    rw -= 3;    //rw = rw-3; この3はどこから来た？ jyuchuはどこからどこまで？ どの画面にある？

    for (i = 0; i <= rw; i++) {

        tmp = parseInt(document.form1.elements['jyuchuname[]'][idx + 8].value.replace(/,/g, ""));   //結果部分？のカンマを削除して数値化。
        if (!isNaN(tmp) && (tmp != null)) {
            //tmpがNaNではない且つnullではないならば、
            sum += tmp;
            //sum=sum+tmp;
        } else {
            //tmpがNaNである。若しくはnullである。
            sum += 0;
            //sum=sum+0;
            //alert("NaN  だよ？");
        }
        idx += NAME_SUU;
        //forが終了した時点ではidx=NAME_SUU*rwとなっているはず。それは何を意味している？ なぜこのように作った？ set_jyuchu_syokei内部ではidxが使われていないように見えるが……？
    }
...
function set_jyuchu_kingaku_1() {
    $('#dialog_item18').val($('#dialog_item27').val()); //受注金額  id="dialog_item18"の内容物の内でid="dialog_item27"の値を取得？ソースコードを見てもかなり離れた位置にあるように思える。
...
function set_jyuchu_kingaku_2() {
    jyu_kin = parseInt($('input[name="jyuchu_syokei"]').val().replace(/,/g, ""));   //カンマ削除
    $('#dialog_item27').val(jyu_kin.toLocaleString()); //受注金額(#dialog_item27)
    //なぜカンマ削除したものにカンマを付けることをしたのだろうか？ （仮説）:parseIntをするのにカンマが邪魔だったから？

    zei = get_jyuchu_zei();
    //関数の定義に従い、get_jyuchu_zeiで手に入るのは10%ならば1.1。8%ならば1.08がreturnされる。
    zei -= 1;   //zei=zei-1; 10%ならば0.1、8%ならば0.08。
    zei_value = Math.round(parseFloat(jyu_kin * zei));
    //jyu_kin*zeiを浮動小数点数を返す⇒小数点第一位で四捨五入する。
    $('#dialog_item28').val(zei_value.toLocaleString()); //消費税(#dialog_item28)

    tmp = parseInt($('input[name="tatekae_jyuchu_syokei"]').val().replace(/,/g, ""));
    //inputタグでname="tatekae_..._syokei"の値からカンマを削除して、整数化。

    if (!isNaN(tmp) && (tmp != null)) {
        tatekae_jyu_kin = parseInt(tmp);    //ここでもparseIntを行う理由は？
    } else {
        tatekae_jyu_kin = 0;
    }
    $('#dialog_item29').val(tatekae_jyu_kin.toLocaleString()); //立替金(#dialog_item29)

    goukei = Math.round(parseFloat(jyu_kin + zei_value + tatekae_jyu_kin));

    $('#dialog_item30').val(goukei.toLocaleString()); //合計(#dialog_item30)
...
function set_syouhizei(type) {
    ...
    rINX = rINXtmp;
    $jyuchu_table = tmpjyuchu;  //何を意味している？ この関数の始めに定義したものを左右入れ替えただけでは？
}
...
function tokuisaki_itiran(item) {
    $("#" + item).on(
        "dblclick", "tr", function () {
            ...
            gyonum = $edit_obj.children("td").eq(0).text(); //tr.children("td")の一番目のテキストデータを取得する。
            if (gyonum != "") {
                ...
                if (tokuisaki_url != "") {
                    ...
                    if (sitensw == 1) {  //inputタイプ
                        ...
                        setHTML += '<input id="client_item_04" type="hidden" size="10" value="' + siten_cd + '" name="bk_tbl_br_code">';    //hiddenなのにsizeとは？
                ...
                if (d1 != "" && d1 !== void 0) {
                    //void 0は未定義であることを意味する。undefinedは書き換えが可能なグローバル変数なのでvoidの方が確実。因みに0であるのは慣例であって決まりではない。
                    kekka = separateDate(d1);   //2278行目付近で定義されている。yyyy/mm/ddに分離するというもの。
                    tmp = get_torihiki(d1); //設定された請求日からの取引条件の取得をする。
                    if (tmp == "") {
                        $('input[name="dt_nyy"]').val("");  //dt_nyyは予定日
                    } else {
                ...
                $('#export_select_item8').val($edit_obj.children("td").eq(1).text());   //id="export_select_item8"に対して$edit_obj.children("td").eq(1).text()をvalとして入力。
        ...
        "dblclick", "tr", function () {
            ...
            if (flg == '1') {
                ...
                if (gyonum != "") {
                    ...
                    rjyuchuINX = ((gidx - JYUCHUNAME_GAICHUID) / NAME_SUU) + 1; //これは何？
...
function getRowNum(id) {
    var myTbl = document.getElementById(id);    //idで指定された要素を取得する。
    var rw = myTbl.rows.length; //↑で取得したモノの行の長さを取得する。
    return rw;
}
function getColNum(id) {
    var myTbl = document.getElementById(id);
    var cl = myTbl.rows[0].cells.length;    //最初の行のセルの数を取得する。
    return cl;
}
function erase(obj) {
    //仮にすべてを表示して未表示とする
...
function insertRow(flg) {
    ...
    if (!rINX || (rINX == '0') || (rINX > rmax)) {
        ...
        errorConfirmDialog('0000', 3);  //3000行を超えた辺りで定義されている。エラーメッセージを表示する関数らしい。    /*;がついていなかった。*/
        return false;
    }
    ...
    var row = table.insertRow(rw + 1);  //943行目で定義されている。
    ...
    //単位のoptionを取得
    optionstr = ajax_func_unitSearch(); //3689行目辺りで定義されている。
    ...
    if ($jyuchu_table == 0) {
        ...
        cell[2].innerHTML = '<td class="j3 tl"><input type="hidden" name="tatekaename[]" value=""><input type="hidden" name="tatekaename[]" value=""><input type="hidden" name="tatekaename[]" value="">' +
            '<input type="text"  size="30"  name="tatekaename[]" value="" ondblclick="javascript:refertatekae_hinmokuButton(' + //ondblclickよりこれは編集（新規作成）画面で使われることが判明。
    ...
    //物件がどちらの状態かで判断
    //$disply_typeで判定
    //$disply_type:jyuchu:受注,gaityu:外注,all:全部
    width_T($disply_type);
    $(function () {
        $('.znkk2')
            .on('keyup', zenkakuConvertWithOutComma);   //znkk2にはカンマが入っていても良く、全角を半角に直す。
    });
    $(function () {
        $('.znkk')
            .on('keyup', zenkakuConvert);   //全角を半角に直す。
    });
}
...
function delete_bukken_Row(type) {
    ...
    justConfirmDialog('本当に削除しますか？', function () {
        var table = document.getElementById(id);
        var rw = rINX;  //流れ的には多分選択された行のことだと思う。
        if (rINX != 1) {
            //削除後に追加する
            table.deleteRow(rw);
            rINX = rINX - 1;
...
function Toggle(id, mojiid) {
    ...
    $('.hide_area').show();
    if (sw == "none") {
        div.style.display = "block";    //swがnoneである場合はstyle.displayに"block"を代入する。
        document.getElementById(mojiid).innerHTML = '<i class="fa fa-minus-square fa-lg"></i>'; //「-」記号がinnerHTMLで入力される。
        width_T("all");
        $('.hide_area').show(); //1247行目でshowしてない？ 新しい表現をしている？
    }
    if (sw == "block") {
...
function width_T(type) {
    ...
    //siblingsは同じ階層のものを全て取得。
    $disply_type = type;    //$display_typeは初めに定義済みの変数。
    if (type == "jyuchu") {
        ...
        erase('.hide_area');    //hide_areaクラスを隠す。
    } else if (type == "gaichu") {
...
function copyDisp() {
    ...
    //確認メッセージを出す関数justConfirmDialogを呼び出す。
    justConfirmDialog('複写しますか？', function () {   //物件一覧画面での「仮登録へコピー」「受注一覧へのコピー」で使うと思われる。
        var tbl = document.getElementById('tbody_ichiran');
        var copyKey = null;
...
function referTokuisakiDialog(obj, formname, type, url) {
    ...
    $("#" + obj).dialog({
        autoOpen: true, //呼ばれると直ぐにダイアログボックスが出現。
        width: 1500,
        height: "auto",
        title: '<i class="fa fa-search"></i>得意先一覧',
        position: {
            of: '#header',              //基点 http://js.studio-kingdom.com/jqueryui/method_overrides/position
            at: 'center bottom',    //基点要素の位置
            my: 'center'        //自分自身の位置
        },
        modal: true,    //モーダル表示（表示中は親画面をいじれない）を行う。
        open: function (event) {
            $('#tokuisaki_item0').val($('#select_item6').val());
            ajax_func(type, url);   //恐らくこれによりダイアログの中でDBの得意先が表示されるものと思われる。
        },
        close: function () {
        }
...
function today_2Month() {
    var date = new Date();
    var format = 'YYYY/MM';
    format = format.replace(/YYYY/g, date.getFullYear());   //dateから年YYYYを取得し、formatのYYYYを置き換える。
    format = format.replace(/MM/g, ('0' + (date.getMonth() + 1)).slice(-2));    //slice(-2)は-2以降に対して処理をするという意味？？ そも、12月を選択するとdate.getMonth()+1⇒13にならない？
    $("#export_select_2Month_item0").val(format);
    $("#export_select_2Month_item13").val(format);  //どこに対して入力？

}
function exportSubmit_2month() {
    //0埋め
    var count = $("#export_select_2Month_item0").val().length;  //上の関数で入力した日付の文字数を取得。
    if (count == 9) {
        $("#export_select_2Month_item0").val($("#export_select_2Month_item0").val() + '1'); //(2019/8/16)のような9文字の場合？？？ 2019/8/161??? count==)は何を意味している？

    } else if (count == 7) {    //count==7は何を意味している？
...
function cancel_date() {
    data_exe_sw = 0;    //1ならば入力、0ならばキャンセルまたはcloseを意味する。この変数を使用する意味はあるか？
...
function input_date() {
    ...
    //hiddenの部分に送信されたものを受け取ってどこぞに表示する仕組み？？
    $("#date-select-dialog").dialog("close");
}
/**
*	日付ダイアログのカレンダー処理
...
function change_date() {
    ...
    setSelectDate(ymd); //hiddenに送信しておくらしい。
}
/*
*  設定された請求日からの取引条件の取得
*  ymd  :  設定された年月日（請求日）
...
function get_torihiki(ymd) {
    ...
    //00でパディング
    m = ("00" + kekka[1]).substr(-2);   //後ろから数えて二つの文字を入手(substr(-2))例えば0012ならば12,008ならば08が返ってくる。結果的に絶対に二桁になるように調整。
    d = ("00" + ddtmp).substr(-2);  //後ろから数えて二つの文字を入手(substr(-2))例えば0012ならば12,008ならば08が返ってくる。結果的に絶対に二桁になるように調整。
...
function getDay() {
    ...
    var m = now.getMonth() + 1; //getMonth()は0～11で返ってくるので実際の月と合わせるには+1が必要です。
    var d = now.getDate();
    //00でパディング
    m = ("00" + m).substr(-2);  //後ろから数えて二つの文字を入手(substr(-2))例えば0012ならば12,008ならば08が返ってくる。結果的に絶対に二桁になるように調整。
    d = ("00" + d).substr(-2);  //後ろから数えて二つの文字を入手(substr(-2))例えば0012ならば12,008ならば08が返ってくる。結果的に絶対に二桁になるように調整。
...
function set_finalDay_yyyymmdd(y, m) {
    var d = getMonthEndDay(y, m);   //y年m月の最終日を入手。
