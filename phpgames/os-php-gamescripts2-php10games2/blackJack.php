...
function evaluateHand($hand) {
    ...
    foreach($hand as $card) {
        $value = intval($value) + intval($faces[$card['face']]);
        //ひとまずここでエースを１１点として計算したときの合計が算出される。
    }
    //以下で点数を修正する。
    while($value > 21 && $aces > 0) {
        //点数が21を超えている、且つ、エースの枚数が0より多い
        $value -= 10;
        $aces -= 1;
        //取り敢えず全エースの値を1として計算し直す。
    }
    return $value;
}
...
?>
<p>現在の貴方の点数: <?php echo evaluateHand($hand); ?></p>
<p>ディーラーは次のカードを見せています。 :  <?php echo $dealer[0]['face'] ?> of <?php echo $dealer[0]['suit'] ?></p> 
<input type='submit' name='submit' value='追加ドロー' />
<input type='submit' name='submit' value='勝負' />
</form>
<a href='blackJack.php'>再戦</a>